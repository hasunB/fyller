...
import { Shield, Briefcase, Activity, Zap, User } from 'lucide-react';
...
export default function RoleBadge({ role }: Role) {
    ...
    const defaultConfig = { color: 'text-gray-500 bg-gray-500/10 border-gray-500/20', icon: User };
    const { color, icon: Icon } = config[role as keyof typeof config] || defaultConfig;

    return (
        <span className={`inline-flex max-w-[130px] items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${color}`}>
